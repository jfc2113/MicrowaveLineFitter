### NOTE:  YOU WILL LIKELY NEED TO CHANGE THE CRITERIA FOR A DIFFERENT DATA SET.
...
def c5(c,c0,rms,imrms,PT_Height,q=False):
	if abs(PT_Height/imrms)>10:
	   if abs(rms)/imrms < 0.75:	##ADDED 2014 26 AUG. 
		return False
	...
	if abs(PT_Height/imrms)>7 and abs(PT_Height/imrms)<10:
	   if abs(rms)/imrms < 1.1:	##ADDED 2014 26 AUG.
		return False
	...
	if abs(PT_Height/imrms)<7:
	   if abs(rms)/imrms < 1.1:
		return False
	   if abs(c-c0)<0.6 and abs(rms/PT_Height)< .10 and abs(rms/imrms) < 1.5:   ##ADDED 2014 26 AUG.
		return False
...
def c7(height,PT_Height,rms,imrms,q=False):
	printFunc(q,  "(GauHeight-DataMax)/imrms=",(height-PT_Height)/imrms,";  rms/imrms=", rms/imrms, "; height/imrms=",height/imrms)
...
def criteria(c,c0,w,rms,height,PT_Height,imrms,BroadF,q=False):
	...
	if c4(rms,height)==True:
	   ...
	   if c5(c,c0,rms,imrms,height)==False:
		...
		return True
	if c7(height,PT_Height,rms,imrms,q=q)==True:
	   printFunc(q,  "In G2criteria, c7 == True")
	   if c5(c,c0,rms,imrms,height)==False:
	  	printFunc(q, "In G2_criteria, c5 == False")
	   else:
	  	printFunc(q, "In G2_criteria, c5 == True")
