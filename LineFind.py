...
def cut(xar,yar,cutSig,Set_rms = 'dummy',startCR=30750.,endCR=49150.,sep=1850.):
	fullcutX = np.zeros((2))
	fullcutY = np.zeros((2))
	rmss=[]
	iii=0
	for freqCenter in np.arange(startCR,endCR+0.2*sep,sep):
		xmask = abs(xar-freqCenter)<sep/2.
		xnow = xar[xmask]
		ynow = yar[xmask]
		if len(xnow) ==0:
			continue
...
def LineFind(f,fd,r_rms='dummy',plotQ='False',cc1=cutchans,rmsthresh0=4,rmsthresh1=3.5,startCR=30750,endCR=49150,sep=1850):
	fLF=f	#frequency, line free
	fdLF=fd	#flux dens, line free
	fLF,fdLF,rmsfinal=cut(fLF,fdLF,rmsthresh0,r_rms,startCR=30750,endCR=49150,sep=1850)		##Do this to evaluate the standard deviation using the Median Absolute Deviation.
	fLF,fdLF=neighborCut(fLF,f,fdLF,fd,2)		##Cut 2 channels before evaluating the standard deviation again.
	fLF,fdLF,rmsff=cut(fLF,fdLF,rmsthresh1,r_rms,startCR=30750,endCR=49150,sep=1850)		##Evaluate the standard deviation using only the non-line data.
	fLF,fdLF,rmsfinal=cut(f,fd,rmsthresh1,rmsff,startCR=30750,endCR=49150,sep=1850)		##Apply the cut to the original data using the calculated rms from the non-line data
